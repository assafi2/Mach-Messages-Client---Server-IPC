...
// can not resolve from given libraries, demanded for virtual address to physi address map
...
using namespace std ;
  ...
  class SaveClientHandler::SaveClient : Client {
		...
		// SHOULD BE APPLIED in sever code as well
...
*/
			...
			save_packet->body.task = cur_task ;
	...
	//		cout << "normal header size : " << sizeof(mach_msg_header_t) << endl ;
			...
			cout << "task " << cur_task << " client sending mach return value  :  " << mr << endl ;
		...
		data_t receive() {
			...
			getBuiltSimplePack() ;
			this->receive_packet->body.task = cur_task ;
			...
			cout << "task " << cur_task << " client task " << cur_task << " sent receive request return value   " << mr << endl ;
			// receiving ...
			buildReplyPack() ;
			mr = mach_msg(
		...
		SaveClient(char* service_name):Client(service_name) {  // call superclass constructor
			...
			cout << "client reply port # : " << reply_port << endl ;
		...
		~SaveClient() {
			...
			cout << "task " << cur_task << " termination msg sent  " << mr << endl ;
